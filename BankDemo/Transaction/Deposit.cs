       ...
       string time = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
        ...
        private void UpdateAmount()
        {
            ...
            try
            {
                using (conn = new SqlConnection(connString))
                {
                    string update = $@"UPDATE tbl_account SET AmountAdd =@AmountAdd WHERE UserID={UserID} and AccNo ='{txtAccNo.Text}'";
                    conn.Open();
                    command = new SqlCommand(update, conn);
                    command.Parameters.AddWithValue(@"AmountAdd", txtFinalA.Text);
        ...
        {
            string a, b, c, Serial;
            string selectStatement = "Select * from tbl_prefix WHERE prefixID ='DOP'";
            int d;
            string update = $"UPDATE tbl_prefix SET SerialNo = {Common.GetData(selectStatement, 0, "SerialNo")}+1 WHERE ID =2";
            using (conn = new SqlConnection(connString))
            {
                conn.Open();
                command = new SqlCommand(update, conn);
